...
class QLearningStrategy:
    ...
    def __init__(self, name: str = "Q-Learning Strategy", 
        ...
        参数:
            ...
            save_path: Q表保存路径
            tournament_id: 锦标赛ID，用于创建特定于锦标赛的模型
        """
        self.name = name
        ...
        # 如果提供了tournament_id，则创建特定于锦标赛的保存路径
        if tournament_id and save_path:
            # 使用锦标赛ID来命名模型文件
            filename = os.path.basename(save_path)
            dirname = os.path.dirname(save_path)
            tournament_filename = f"tournament_{tournament_id}_{filename}"
            self.save_path = os.path.join(dirname, tournament_filename)
        else:
        ...
        if self.save_path and os.path.exists(self.save_path):
            try:
                ...
                print(f"已加载Q表 {self.save_path}，包含{len(self.q_table)}个状态")
    ...
    def save_q_table(self):
        ...
        if self.save_path:
            try:
                ...
                print(f"Q表和学习数据已保存到 {self.save_path}，包含{len(self.q_table)}个状态")
            except Exception as e:
                print(f"保存Q表失败: {e}")
    
    @staticmethod
    def load_tournament_data(save_path, tournament_id):
        """
        加载特定锦标赛的Q学习数据
        ...
        dirname = os.path.dirname(save_path)
        filename = os.path.basename(save_path)
        tournament_path = os.path.join(dirname, f"tournament_{tournament_id}_{filename}")
        
        if os.path.exists(tournament_path):
            try:
                with open(tournament_path, 'rb') as f:
                    data = pickle.load(f)
                print(f"已加载锦标赛 {tournament_id} 的Q学习数据")
                return data
            except Exception as e:
                print(f"加载锦标赛数据失败: {e}")
...
def make_move(opponent_history, tournament_id=None):
    ...
    global ql_strategy
    if 'ql_strategy' not in globals() or (tournament_id is not None and getattr(ql_strategy, 'tournament_id', None) != tournament_id):
        ...
        ql_strategy = QLearningStrategy(save_path=save_path, tournament_id=tournament_id)
    
    # 更新上一轮对手选择
    if opponent_history and 'last_history_length' in globals():
        if len(opponent_history) > last_history_length:
...
if __name__ == "__main__":
    # 创建Q-learning策略
    ql = QLearningStrategy(tournament_id=999)  # 测试用的锦标赛ID
